        ...
        egui::CentralPanel::default().show(ctx, |ui| {
            ui.vertical(|ui| {
                ui.horizontal(|ui| {
                    ...
                    ui.checkbox(&mut self.timestamp, "Time")
                        .on_hover_text("Append timestamp to the received data");
                    ui.checkbox(&mut self.scrolling_locked, "Lock scrolling");
                });

                egui::containers::ScrollArea::vertical()
                    .auto_shrink([false, false])
                    .stick_to_bottom(self.scrolling_locked)
                    .max_height(400f32)
            ...
            if let Ok(s) = self.serial.get_receiver().try_recv() {
                self.current_text.push_str(&format!(
                    "{} {}",
                    if self.timestamp {
                        chrono::Local::now().format("%H:%M:%S:%f>").to_string()
                    } else {
                        "".to_string()
                    },
                    s
            ...
            ui.label(format!("len = {}", self.current_text.len()));
        });
